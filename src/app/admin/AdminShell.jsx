    ...
    if (user?.role === "admin") {
      Promise.all([
        ...
        .then(([ordersData, reviewsData]) => {
          setBadgeCounts({
            orders:  ordersData.count > 0 ? String(ordersData.count) : null,
            reviews: reviewsData.count > 0 ? String(reviewsData.count) : null,
          });
        })
        .catch(console.error);
    }
          ...
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton asChild>
                <Link href="/"><ChevronRight size={16} /><span>Дэлгүүр харах</span></Link>
              </SidebarMenuButton>
            </SidebarMenuItem>
            <SidebarMenuItem>
              <SidebarMenuButton
